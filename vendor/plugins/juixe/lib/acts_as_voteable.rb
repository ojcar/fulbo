      ...
      module InstanceMethods
        def votes_for
          votes = Vote.find(:all, :conditions => [
            "voteable_id = ? AND voteable_type = 'Entry' AND vote = TRUE",
            id, #votes.type.name #self.type.name
          ])
          votes.size
        end
        ...
        def votes_against
          votes = Vote.find(:all, :conditions => [
            "voteable_id = ? AND voteable_type = 'Entry' AND vote = FALSE",
            id, #votes.type.name #self.type.name
          ])
          votes.size
        end
        ...
        def what_vote?(user)
        	voto = Vote.find(:first, :conditions => ["user_id = ? AND voteable_id = ?", user.id, id])
        	voto.vote
        end
